...
import {selectLinks, increasePoint, decreasePoint} from "../components/links/linksSlice";
...
export default function Home() {
    ...
    const renderList = () => {
        ...
        return result.map((item) => {
            return (
                <LinkItem id={item.id} name={item.name} points={item.points} key={item.id} onIncreaseClick={handleIncreaseClick} onDecreaseClick={handleDecreaseClick} />
            )
        })
    }
